...
export class AbstractTheme {
  ...
  getFormControl (label, input, description, infoText): HTMLElement  {
    const el = document.createElement('div')
  ...
  getCheckboxDescription (text: any): HTMLElement {
    return this.getDescription(text)
  }

  getFormInputDescription (text: any): HTMLElement {
    return this.getDescription(text)
  }
  ...
  getFormButtonHolder (buttonAlign?): HTMLElement  {
    return this.getButtonHolder()
  }

  getButton (text, icon, title): HTMLElement  {
  ...
  getFormButton (text, icon, title) : HTMLElement {
  ...
  getTableHeaderCell (text) : HTMLElement {
    const el = document.createElement('th')
    el.textContent = text
    return el
  }

  getTableCell () : HTMLElement  {
    const el = document.createElement('td')
    return el
  }

  getErrorMessage (text) : HTMLElement {
    const el = document.createElement('p')
    //el.style = el.style || {}
    el.style.color = 'red'
    el.appendChild(document.createTextNode(text))
  ...
  getProgressBar(): HTMLElement {
    const max:string = '100'; const start:string = '0'
